...
class Car:
    def __init__(self, x, y, space):
        ...
        # Spring Joints
        f_spring = pymunk.DampedSpring(
            chassis_body, f_wheel_body, (50, -35), (0, 0),
            rest_length=50, stiffness=100, damping=10
        )
        r_spring = pymunk.DampedSpring(
            chassis_body, r_wheel_body, (-50, -35), (0, 0),
            rest_length=50, stiffness=100, damping=10
        )

        # motors
        f_motor = pymunk.SimpleMotor(chassis_body, f_wheel_body, 12)
        r_motor = pymunk.SimpleMotor(chassis_body, r_wheel_body, 12)

        space.add(chassis_body, chassis_shape)
        space.add(f_wheel_body, f_wheel_shape)
        space.add(r_wheel_body, r_wheel_shape)
        space.add(f_joint, r_joint)
        space.add(f_spring, r_spring)
        space.add(f_motor, r_motor)
...
class App(arcade.Window):
    def __init__(self):
        ...
        self.car = Car(50, 200, self.space)
        self.lines = []
        self.add_static()
        # sprites
        self.sprites = arcade.SpriteList() 
    ...
    def on_draw(self):
        ...
        for line in self.lines:
            arcade.draw_line(*line, arcade.color.WHITE, 2)
    ...
    def add_static(self):
        ...
        # obstculos
        self.add_static_segment(200, 0, 400, 50)
        self.add_static_segment(400, 50, 600, 50)
        self.add_static_segment(600, 50, 800, 0)
        
        # self.add_static_segment(450, 0, 550, 150)
        # self.add_static_segment(550, 150, 650, 0)

        # self.add_static_segment(700, 0, 750, 50)
        # self.add_static_segment(750, 50, 800, 0)
